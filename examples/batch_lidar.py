    ...
    with RenderApp() as render:
        ...
        def render_step():
            ...
            if render_cnt % lidar_render_substep == 0:
                if dynamic_lidar:
                    # rays_theta, rays_phi = livox_generator.sample_ray_angles(downsample=4)
                    rays_theta, rays_phi = livox_generator.sample_ray_angles(downsample=1)
                st = time.time()
                distances, local_rays_batch = lidar_wrapper.trace_rays_batch(
                    geom_xpos_batch_jax,
                    geom_xmat_batch_jax,
                ...
                if _Profile.value.lower() == "true":
                    ...
                    # Check if we've reached the test count
                    if lidar_test_cnt >= test_count:
                        # Print statistics
